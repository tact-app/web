...
const getDescriptionsApi = (apiService: ApiService) => ({
  ...
  get: (id: string) => apiService.get<GoalDescriptionData>(`/api/description`, { id }),
  update: (data: { id: string, fields: Partial<GoalDescriptionData> }) => apiService.put<GoalDescriptionData>(`/api/description`, data),
});
