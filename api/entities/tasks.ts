...
const getTasksApi = (apiService: ApiService) => ({
  list: (id: string) => apiService.get<{ tasks: Record<string, TaskData>, order: string[] }>(`/api/tasks`, { id }),
  create: (task: TaskData) => apiService.post<TaskData[]>(`/api/tasks/create`, task),
  delete: (listId: string, ids: string[]) => apiService.delete<TaskData[]>(`/api/tasks/delete`, { ids, listId }),
