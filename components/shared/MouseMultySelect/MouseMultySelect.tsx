...
export const MouseMultySelect = ({
  ...
  const handleMoueMove = (e: MouseEvent) => {
    ...
    if (!isOpenRef.current && mouseMoveCheckToStart(myPositionRef.current, pageX, pageY, minFramePx)) {
    ...
    handleSelection(
      ...
      { minItemPx, selectedItemClassName, isOpenRef, saveSelectAfterFinish }
    )()
  ...
  const handleMouseUp = (e: MouseEvent) => {
    ...
    onFinishSelection?.(selectedElement, e);
    setIsOpen(false);
    clearTimer();
  };
  ...
  const handleMouseDown = (e: MouseEvent) => {
    ...
    if (notStartWithSelectableElements) {
      const elementInitiator = e.composedPath().find((element: Element) => element?.classList?.contains(itemClassName));
